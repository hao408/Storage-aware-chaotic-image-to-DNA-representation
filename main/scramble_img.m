function scrambled_img = scramble_img(original_img, key_stream_scrambling)
    ...
    red_img = scrambled_img(:,:,1);
    green_img = scrambled_img(:,:,2);
    blue_img = scrambled_img(:,:,3);
    
    % Convert to linear indexing for easier manipulation
    red_img_linear = red_img(:);
    green_img_linear = green_img(:);
    blue_img_linear = blue_img(:);
    
    % Perform scrambling using temporary array
    ...
    % Reshape back to 2D
    scrambled_img(:,:,1) = reshape(temp_red, [M, N]);
    scrambled_img(:,:,2) = reshape(temp_green, [M, N]);
    scrambled_img(:,:,3) = reshape(temp_blue, [M, N]);
end